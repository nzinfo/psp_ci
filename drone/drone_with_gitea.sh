...
# TODO: change to drone.pdev
CI_SERVER="http://ci.local:8000"

CI_OAUTH_APP="drone"

# 需要在 docker 的宿主机执行，假设 数据 在 
ACCESS_TOKEN_CMD="head -1 ${DATA_PATH}/gitea/gitea_admin_resp.txt | sed 's/.*... //'"
...
DRONE_APP_ID=`bash -c "curl -X GET \"${GIT_SERVER}/api/v1/user/applications/oauth2\" \
...
then
    ...
    PAYLOAD_CREATE_DRONE_APP="{ \"name\": \"${CI_OAUTH_APP}\", \"redirect_uris\": [ \"${GIT_SERVER}/login\" ] }"
